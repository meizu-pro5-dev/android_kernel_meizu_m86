...
struct bpf_prog *bpf_int_jit_compile(struct bpf_prog *prog)
{
	...
	*/
	...
	/* Never publish executable code while its backing pages remain
	 * writable.  A page-attribute failure is a hard JIT failure; the
	 * caller will retain the verified interpreter program instead. */
	ret = set_memory_ro((unsigned long)header, header->pages);
	if (ret) {
		int rw_ret;

		pr_err("failed to make JIT image read-only: ret=%d addr=%pK pages=%u\n",
			ret, header, header->pages);
		/* apply_to_page_range() may have changed a prefix before failing.
		 * Restore the complete allocation before returning it to the module
		 * allocator.  If that cannot be guaranteed, retain the allocation:
		 * leaking private pages is safer than recycling partially read-only
		 * pages into an allocator which expects writable memory. */
...
void bpf_jit_free(struct bpf_prog *prog)
{
	...
	if (ret) {
		pr_err("failed to make JIT image writable; retaining allocation: ret=%d addr=%pK pages=%u\n",
			ret, header, header->pages);
		goto free_filter;
	}
	bpf_jit_binary_free(header);
