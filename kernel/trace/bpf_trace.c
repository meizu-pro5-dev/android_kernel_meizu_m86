...
/* 3.10 lacks strncpy_from_unsafe().  Read one byte at a time so a bad
 * address fails closed without dereferencing arbitrary kernel memory. */
static long bpf_strncpy_from_unsafe(char *dst, const void *src, long count)
{
	long i;
	char c;
	...
	for (i = 0; i < count; i++) {
		if (probe_kernel_read(&c, (const char *)src + i, 1))
			return -EFAULT;
		dst[i] = c;
		if (!c)
			return i;
...
BPF_CALL_3(bpf_probe_read_str, void *, dst, u32, size, const void *, unsafe_ptr)
{
	 ...
	 */
	ret = bpf_strncpy_from_unsafe(dst, unsafe_ptr, size);
	if (unlikely(ret < 0))
		memset(dst, 0, size);
