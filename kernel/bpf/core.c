...
static void bpf_prog_free_deferred(struct work_struct *work)
{
	...
	/* An arch eBPF JIT owns executable memory until this deferred release. */
#ifdef CONFIG_BPF_JIT
...
void bpf_user_rnd_init_once(void)
{
	...
	/* 3.10 has no prandom_init_once()/prandom_seed_full_state(). */
...
/* The legacy classic socket-filter ABI remains available even when this
 * architecture only supplies an eBPF JIT. */
#ifdef CONFIG_BPF_JIT
void __weak bpf_classic_jit_compile(struct sk_filter *fp)
{
}

void __weak bpf_classic_jit_free(struct sk_filter *fp)
{
}
#endif
