...
/*
 * NMI-safe read for an event that is local to the current task or CPU.
 * BPF perf-event maps use this path and must never force cross-CPU I/O.
 */
			       ...
			       struct task_struct *task)
{
	...
	if (bpf_prog_array_valid(call)) {
		/* The tracepoint BPF ABI hides this register pointer. */
		*(struct pt_regs **)raw_data = regs;
		if (!trace_call_bpf(call, raw_data) || hlist_empty(head)) {
			perf_swevent_put_recursion_context(rctx);
			return;
		}
		/* Restore the user-visible trace record header after dispatch. */
		perf_trace_buf_update(raw_data, call->event.type);
	}
#endif
