...
/*
 * Keep the 3.10 classic socket-filter layout intact.  The eBPF runtime
 * lives beside it in struct bpf_prog; old socket filters and seccomp filters
 * still own this object directly.
 */
struct sk_filter
{
	atomic_t		refcnt;
	...
	struct rcu_head	rcu;
	/* NULL for the legacy classic filter, otherwise an eBPF program. */
...
{
	if (fp->prog)
		return sizeof(*fp) + bpf_prog_size(fp->prog->len);
...
#define SK_RUN_FILTER(FILTER, SKB) \
	((FILTER)->prog ? bpf_prog_run_save_cb((FILTER)->prog, \
					(struct sk_buff *)(SKB)) : \
...
/* Classic BPF interpreter opcodes used by seccomp and socket filters. */
enum {
	BPF_S_RET_K = 1,
	BPF_S_RET_A,
	BPF_S_ALU_ADD_K,
...
/*
 * Linux 3.10's CONFIG_BPF_JIT is the classic socket-filter JIT.  Keep its
 * ABI separate from the eBPF runtime below: net/core/filter.c passes a
 * struct sk_filter here.
 */
extern void bpf_classic_jit_compile(struct sk_filter *fp);
extern void bpf_classic_jit_free(struct sk_filter *fp);

/* eBPF JIT entry points.  These deliberately use different symbols from
 * the legacy classic socket-filter hooks above. */
struct bpf_prog *bpf_int_jit_compile(struct bpf_prog *prog);
...
#else
/* Classic BPF JIT hooks are part of the pre-existing 3.10 ABI. */
static inline void bpf_classic_jit_compile(struct sk_filter *fp)
{
}

static inline void bpf_classic_jit_free(struct sk_filter *fp)
{
}
